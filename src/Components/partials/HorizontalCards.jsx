...
const HorizontalCards = ({ shows }) => {
  return (
    ...
    <div className='h-48  w-full overflow-x-auto flex gap-3'>
      {shows.map((show, i) => <Link to={`/${show.media_type}/details/${show.id}`} key={i}  className={`rounded relative overflow-hidden h-full w-[13vw] flex items-end p-3 shrink-0 bg-gray-900 text-white`}>
