...
const FilterTrending = ({title, state, option, setstate }) => {
    return (
        <div className='flex items-center gap-2'>
            <h1 className='text-white shrink-0 font-semibold'>{title} : </h1>
            <select value={state} onChange={(e) => setstate(e.target.value)} className='px-2 py-1 capitalize bg-transparent border rounded text-zinc-100 font-semibold' id="">
